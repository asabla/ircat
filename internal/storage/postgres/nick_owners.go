...
// nickOwnerStore is the Postgres implementation of [storage.NickOwnerStore].
...
const (
	...
	noSelectOne       = noSelectAll + ` WHERE nick = $1`
	noSelectByAccount = noSelectAll + ` WHERE account_id = $1 ORDER BY is_primary DESC, nick`
)
...
func (s *nickOwnerStore) Delete(ctx context.Context, nick string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM nick_owners WHERE nick = $1`, nick)
	if err != nil {
