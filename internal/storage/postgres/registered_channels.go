...
// registeredChannelStore is the Postgres implementation of
...
func (s *registeredChannelStore) Delete(ctx context.Context, channel string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM registered_channels WHERE channel = $1`, channel)
	if err != nil {
...
const (
	...
	caSelectOne = caSelectAll + ` WHERE channel = $1 AND account_id = $2`
	caSelectCh  = caSelectAll + ` WHERE channel = $1 ORDER BY account_id`
)
...
func (s *registeredChannelStore) DeleteAccess(ctx context.Context, channel, accountID string) error {
	res, err := s.db.ExecContext(ctx,
		`DELETE FROM channel_access WHERE channel = $1 AND account_id = $2`,
		channel, accountID,
	)
	if err != nil {
