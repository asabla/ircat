...
// memoStore is the SQLite implementation of [storage.MemoStore].
...
const (
	...
	memoSelectOne = memoSelectAll + ` WHERE id = ?`
)
...
func (s *memoStore) ListForRecipient(ctx context.Context, recipientID string) ([]storage.Memo, error) {
	rows, err := s.db.QueryContext(ctx,
		memoSelectAll+` WHERE recipient_id = ? ORDER BY created_at ASC`, recipientID)
	if err != nil {
...
func (s *memoStore) MarkRead(ctx context.Context, id string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE memos SET read = 1 WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("memos.MarkRead: %w", err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("memos.MarkRead rows: %w", err)
	}
	if n == 0 {
...
func (s *memoStore) Delete(ctx context.Context, id string) error {
	...
	n, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("memos.Delete rows: %w", err)
	}
	if n == 0 {
		return storage.ErrNotFound
	}
...
func (s *memoStore) PurgeOlderThan(ctx context.Context, before time.Time) (int, error) {
	res, err := s.db.ExecContext(ctx, `DELETE FROM memos WHERE created_at < ?`, before)
	if err != nil {
		return 0, fmt.Errorf("memos.PurgeOlderThan: %w", err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf("memos.PurgeOlderThan rows: %w", err)
	}
	return int(n), nil
}
...
func (s *memoStore) CountUnread(ctx context.Context, recipientID string) (int, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM memos WHERE recipient_id = ? AND read = 0`, recipientID).Scan(&count)
	if err != nil {
