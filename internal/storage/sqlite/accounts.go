...
// accountStore is the SQLite implementation of [storage.AccountStore].
...
const (
	...
	acctSelectOne  = acctSelectAll + ` WHERE username = ?`
	acctSelectByID = acctSelectAll + ` WHERE id = ?`
)
...
func (s *accountStore) Update(ctx context.Context, acct *storage.Account) error {
	...
	res, err := s.db.ExecContext(ctx,
		`UPDATE accounts
		   SET password_hash = ?, email = ?, verified = ?, updated_at = ?
		 WHERE username = ?`,
	...
	if err != nil {
		return fmt.Errorf("accounts.Update rows: %w", err)
	}
	if n == 0 {
...
func (s *accountStore) Delete(ctx context.Context, username string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM accounts WHERE username = ?`, username)
	if err != nil {
		return fmt.Errorf("accounts.Delete: %w", err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("accounts.Delete rows: %w", err)
	}
	if n == 0 {
		return storage.ErrNotFound
	}
