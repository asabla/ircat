...
// tokenStore implements [storage.TokenStore] over the api_tokens
// table. The plaintext token is never stored — only the sha256 hex
// hash. The application is responsible for hashing before calling
// Create.
...
const (
	...
	tokenSelectOne = tokenSelectAll + ` WHERE id = ?`
	tokenSelectByH = tokenSelectAll + ` WHERE hash = ?`
)

func (s *tokenStore) Get(ctx context.Context, id string) (*storage.APIToken, error) {
	return s.scanOne(ctx, tokenSelectOne, id, "Get", id)
}

func (s *tokenStore) GetByHash(ctx context.Context, hash string) (*storage.APIToken, error) {
	return s.scanOne(ctx, tokenSelectByH, hash, "GetByHash", hash)
}

// scanOne factors out the single-row scan path so Get and GetByHash
// share the timestamp / NULL handling.
func (s *tokenStore) scanOne(ctx context.Context, query, key, op, identifier string) (*storage.APIToken, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("tokens.%s %q: %w", op, identifier, err)
	}
...
func (s *tokenStore) Create(ctx context.Context, token *storage.APIToken) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO api_tokens(id, label, hash, scopes, created_at) VALUES (?, ?, ?, ?, ?)`,
...
func (s *tokenStore) TouchLastUsed(ctx context.Context, id string, at time.Time) error {
	res, err := s.db.ExecContext(ctx,
		`UPDATE api_tokens SET last_used_at = ? WHERE id = ?`, at.UTC(), id)
	if err != nil {
